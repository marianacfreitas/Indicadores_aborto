...
dir.create("arquivos_TISS_DET", showWarnings = FALSE)
...
for (ano in anos) {
  ...
  for (uf in ufs) {
    for (mes in 1:12) {
      ...
      filename <- paste0("arquivos_TISS_DET/", uf, "_", ano, mes_str, "_HOSP_DET.zip")
      download.file(url, filename)
      unzip(filename, exdir = "arquivos_TISS_DET")
      # }
    }
  }
}
...
arquivos <- list.files("arquivos_TISS_DET", full.names = TRUE, pattern = "\\.csv", recursive = TRUE)
...
dados_concatenados_hosp_det <- bind_rows(dados) 

write.csv(dados_concatenados_hosp_det, "dados_ANS_hosp_det_2015.csv")

###UM NOVO DIA, COMECE AQUI - FILTRAGEM TODOS CASOS ANS#######################
# dados_concatenados_hosp_det <- read.csv("dados_ANS_hosp_det_2015.csv")
...
dados_filtrados_cid <- read.csv("dados_filtrados_CIDs_ANS_2015_hosp.csv")
...
write.csv(dados1, "dados_ANS_2015_abortos.csv")


###UM NOVO DIA, COMECE AQUI - FILTRAGEM SÓ ABORTO ANS #######################
dados1 <- read.csv("dados_ANS_2015_abortos.csv")
...
write.csv(dados_uf_fet_pivotado, file = "dados_ANS_aborto_2015_tabela.csv", row.names = FALSE)
